...
def create_distribution(x, n_bins=10):
    min, max = torch.min(x), torch.max(x)
    x= (x - min) / (max - min)
    histogram = torch.histc(x, bins=n_bins, min=0, max=1)
...
def plot_scores(scores, x_label, y_label):
...
def plot_heatmap(x, x_label, y_label):
...
def simb_score(dataset, n_bins=10, features_to_skip=[], device='cpu'):
    ...
    for feature in range(n_features):
        ...
        distribution = create_distribution(dataset_slice, n_bins=n_bins)
...
def stood_score(train_set, validation_set, test_set, n_bins=10, features_to_skip=[], device='cpu'):
    ...
    for feature in range(n_features):
        ...
        train_distribution = create_distribution(train_slice, n_bins=n_bins)
        validation_distribution = create_distribution(validation_slice, n_bins=n_bins)
        test_distribution = create_distribution(test_slice, n_bins=n_bins)
...
def ios_score(features, labels, features_to_skip=[], device='cpu'):
    ...
    mean_deltas = torch.zeros(n_features - len(features_to_skip), n_labels)
